...
export default function Pricing() {
 ...
 <section className="pad section-ivory"><div className="wrap"><div className="spaces">{spaces.map(space=><a key={space.key} className="space-card" href={`/home/booking?space=${space.key}`}><div className="img"><Image width={1200} height={800} sizes="(max-width: 820px) 100vw, 50vw" src={`/collabix/img/photos/${space.image}`} alt={space.name}/></div><div className="body"><span className="kicker">Up to {space.capacity} {space.capacity===1?"person":"people"}</span><h2>{space.name}</h2><p>{space.description}</p><div className="price"><span className="amt">{money(space.rate)}</span><span className="per">/ hour · before tax</span><span className="book">Explore →</span></div></div></a>)}</div><p style={{marginTop:"2rem"}}>Need a monthly desk or a team plan? <a className="link-arw" href="mailto:connect@collabix.co.in">Get in touch →</a></p></div></section></>;
}
